...
class Crossover:
    ...
    def calc(self, pop, n_var):
        ...
        for i in range(parents.shape[0]):
            child.append(np.concatenate(
                [d[i, 0, :crossPoint[i]], d[i, 1, crossPoint[i]:]]))
...
class PolynomialMutation:
    ...
    def calc(self, x, xl, xu):
        ...
        xl = np.repeat(xl[None, :], X.shape[0], axis=0)[
            do_mutation]  # selecting who is mutating
...
class NSGAII(_algorithm):
    ...
    def fastSort(self, x):
        ...
        # if i ==  0, i is non-dominated, set i rank to 1, i belongs to first
        # non-dominated front
        Np[nDom == 0] = 1
        k = 1
        ...
        while np.sum(Np == 0) > 0:
            ...
            # and Np == 0 ensure that we don't pass over the first ranked
            # non-dom solutions
            Np[(nDom == 0) & (Np == 0)] = k
    ...
    def crowdDist(self, x):
        ...
        dist = np.vstack([X, np.full(nobj, np.inf)]) - \
            np.vstack([np.full(nobj, -np.inf), X])

        norm = np.max(X, axis=0) - np.min(X, axis=0)
        dist_to_last, dist_to_next = dist, np.copy(dist)
        dist_to_last, dist_to_next = dist_to_last[:-
                                                  1] / norm, dist_to_next[1:] / norm
        J = np.argsort(ord, axis=0)
        ret = np.sum(dist_to_last[J, np.arange(nobj)] +
                     dist_to_next[J, np.arange(nobj)], axis=1) / nobj
    ...
    def crowdDist2(self, x):
        ...
        for obj in range(x.shape[1]):
            ...
            for i in range(1, n - 1):
                dist[i] = dist[ord[i]] + \
                    (x[ord[i + 1], obj] - x[ord[i - 1], obj]) / norm
    ...
    def sample(self, generations, n_obj, n_pop=None,
        ...
        for igen in range(1, self.generations - 1):
            ...
            if self.skip_duplicates:
                ...
                for rk in range(1, np.max(nonDomRank) + 1):
                    crDist[nonDomRank == rk] = self.crowdDist(
                        Of[nonDomRank == rk, :])
            else:

                n_pop_combined = self.n_pop * 2
                # evaluate population
                param_generator = ((i, Rt[i, :])
                                   for i in range(n_pop_combined))
            ...
            Qt = self.crossover.calc(pop=Qt, n_var=n_var)
            # mutation
            Qt = self.mutation.calc(
                x=Qt, xl=self.varminbound, xu=self.varmaxbound)
