...
class mcmc(_algorithm):
    ...
    def sample(self, repetitions, nChains=1):
        ...
        print(
            'Starting the MCMC algotrithm with ' +
            str(repetitions) +
            ' repetitions...')
        ...
        self.bestpar = np.array(
            [[np.nan] * len(self.stepsizes)] * self.nChains)
        ...
        self.iter = 0
        param_generator = ((curChain, self.parameter()[
                           'random']) for curChain in range(int(self.nChains)))
